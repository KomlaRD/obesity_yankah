...
obesity%>%
  select(age_years, hours_sleep, fbs_today, fbs_last_visit, sbp, dbp, height, weight, body_fat, muscle_mass, visceral_fat, waist_circumference, bmi)%>%ggpairs(columnLabels = c("age", "hr", "fbs0", "fbs1", "sbp", "dbp", "ht", "wt", "bf", "mm", "vf", "wc", "bmi"))
```
...
anthropometry%>%
        ...
        modify_caption("**Table 2. Anthropometry**")%>%
        bold_labels()%>%
  add_p()%>%
  bold_p()
```
...
clinical%>%
        ...
        modify_caption("**Table 1. Clinical**")%>%
        bold_labels()
```
...
dietary%>%
        tbl_summary()%>%
        modify_caption("**Table 4. Dietary**")%>%
        bold_labels()
```
...
physical%>%
        tbl_summary()%>%
        modify_caption("**Table 5. Physical activity**")%>%
        bold_labels()
```
