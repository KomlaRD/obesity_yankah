...
# Replace value of 729 with 72.9
obesity$weight[obesity$ weight == 729] <- 72.9

# Replace value of 967 with 96.7
obesity$weight[obesity$weight == 967] <- 96.7

# Replace unusual height values
obesity$height[obesity$height == 14] <- 140
...
obesity%>%
  select(age_years, hours_sleep, fbs_today, fbs_last_visit, sbp, dbp, height, weight, body_fat, muscle_mass, visceral_fat, waist_circumference, bmi)%>%
  ggcorr(label = TRUE)
...
anthropometry%>%
        ...
        modify_caption("**Table 5. Anthropometry by outcome**")%>%
        bold_labels()
```
...
physical%>%
        tbl_summary(by = sex)%>%
        modify_caption("**Table 13. Physical activity**")%>%
        bold_labels()%>%
  add_p()%>%
  bold_p()
```
...
physical%>%
        ...
        modify_caption("**Table 14. Physical activity**")%>%
        bold_labels()%>%
  add_p()%>%
  bold_p()
```
...
#full_model <- glm(dm$dm ~ dm$age + dm$chol + dm$insurance, family=binomial (link=logit)) 
#summary(full_model)
...
```
```{r}
obesity%>%select(weight, height, bmi, bmi_outcome, bmi_cat)%>%filter(is.na(bmi_outcome))
```
