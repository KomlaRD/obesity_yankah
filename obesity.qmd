...
```{r age-model, eval=FALSE}
age_model <- glm(obesity$bmi_obese ~ obesity$age_years, family = binomial(link = logit))
```
...
full_model <- glm(obesity$bmi_obese ~ obesity$age_years + obesity$sex + obesity$ethnicity + obesity$occupation, family=binomial (link=logit)) 
